...
function main() {
  ...
  const filename = path.join(__dirname, "gen_files/dropTicket/distro.csv");

  // what file should we write the merkle proofs too?
  const output_file = path.join(__dirname, "gen_files/dropTicket/merkleProof.json");

  //file that has the user claim list
  const userclaimFile = path.join(
    __dirname,
    "gen_files/dropTicket/claimList.json"
  );

  //contract of items being sent out
  const airdropContract = "0x158030fd081e1A5C85Bd09B0b8E2Bf354895B222";
